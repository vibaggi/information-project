function [I] = CEH_VBA_v4(T, Tn, USUST,USUSE, dt, valCriterio,sigma)
...
dtUSUST = (dt*median(USUST));
rngUSUST = 0:dtUSUST:max(USUST);	
sigmaUSUST = sigma*median(USUST);
gaussUSUST = dtUSUST/sqrt(2*pi())/sigmaUSUST*exp(-0.5*((rngUSUST-mean(rngUSUST))/sigmaUSUST).^2);
...
countUSUST = histc(USUST, rngUSUST);
countUSUSE = histc(USUSE, rngUSUST);
...
d2 = conv(countUSUST,gaussUSUST,'same');   % convolves the histogram counts with a gaussian for smoothig
d3 = conv(countUSUSE,gaussUSUST,'same');

d = d/length(T)/dt;				% normalizing
dn = dn/length(Tn)/dt;		
d2 = d2/length(USUST)/dtUSUST;
d3 = d3/length(USUSE)/dtUSUST;
...
entropyUSUST = -dtUSUST*sum(d2.*log2(d2));
entropyUSUSE = -dtUSUST*sum(d3.*log2(d3));

I = (entropyUSUST - entropyT) + (entropyUSUSE - entropyTn);
